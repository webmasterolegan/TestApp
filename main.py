...
for input_file in inputDirectory.glob('*.csv'):
    with open(input_file, encoding='cp1251') as File:
        ...
        table_headers = list(
            merge(table_headers, email_headers, phone_headers))

        # Файл итоговой таблицы
        output_file_name = str(input_file).replace(
            inputPath + '/', outputPath + '/result_')
